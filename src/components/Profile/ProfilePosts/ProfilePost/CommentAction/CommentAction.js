...
import s from './CommentAction.module.scss';

const CommentAction = ({ onSetCommentActive, commentsLength }) => {

   const commnetsLengthCount = (commentsLength) => {
      if (commentsLength <= 0) {
         return '';
      } else return commentsLength;
   };

   return (
      <div className={s.comment}>
         <button onClick={() => onSetCommentActive(true)}>
         ...
         <span>{commnetsLengthCount(commentsLength)}</span>
      </div>
   );
}; 
